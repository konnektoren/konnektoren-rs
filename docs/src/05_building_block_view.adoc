...
The GameController is a central component that manages the game state, handles commands, and coordinates events within the system.

[plantuml, game-controller-structure, png]
----
@startuml
...
- Handles and publishes commands
- Coordinates events through the EventBus
...
=== Level 3: Yew Game Controller Provider

The Game Controller Provider is a crucial component in the Konnektoren Yew frontend application. It manages the game state and provides access to the game controller throughout the component tree.

[plantuml, game-controller-provider, png]
----
@startuml
package "Frontend" {
...
The provider also includes several custom hooks to easily access different parts of the game state:
...
This structure allows for efficient state management and easy access to game-related functionality throughout the Yew application.
